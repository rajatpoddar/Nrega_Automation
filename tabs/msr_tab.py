...
def create_tab(parent_frame, app_instance):
    ...
    # --- Work Codes Tab ---
    work_codes_frame.columnconfigure(0, weight=1)
    work_codes_frame.rowconfigure(0, weight=1)
    ...
    cols = ("Workcode", "Status", "Timestamp")
...
def _process_single_work_code(app, driver, wait, work_key):
    try:
        # Proactively clear any stale alerts before starting
        try:
        ...
        # UPDATED: More robust post-save handling loop
        outcome_found = False
        for _ in range(3): # Retry checks for a few seconds
            # Priority 1: Check for any alert
            try:
                ...
                # Priority 2: Check for known page-level error
                if "Expenditure on unskilled labours exceeds sanction amount" in driver.page_source:
                    ...
                    outcome_found = True
                    break
                # If no alert and no error, page might be loading, so we wait
                time.sleep(1)
        ...
        app.after(0, update_status, f"✅ {work_key} done. Waiting {delay:.1f}s...")
        time.sleep(delay)
        ...
        if isinstance(e, IndexError) and "Muster Roll" in error_str:
            display_msg = "Muster Roll not found."
        elif isinstance(e, TimeoutException):
            display_msg = "Page timed out or element not found."

        _log_result(app, "failed", work_key, display_msg)
    except Exception as e:
        # This will catch the UnexpectedAlertPresentException and other critical errors
...
def _log_result(app, status, work_key, msg):
    ...
    app.log_message(widgets['log_display'], f"'{work_key}' — {status.upper()}: {log_msg}", level=level)
    app.after(0, lambda: widgets['results_tree'].insert("", "end", values=(work_key, log_msg, timestamp)))
