# tabs/wc_gen_tab.py (Upgraded to CustomTkinter)
import tkinter
...
def create_tab(parent_frame, app_instance):
    ...
    widgets['panchayat_entry'] = ctk.CTkEntry(controls, width=30)
    widgets['panchayat_entry'].grid(row=0, column=1, sticky="ew", pady=10, padx=15)
    ...
    widgets['select_button'] = ctk.CTkButton(file_frame, text="Select workcode_data.csv", command=lambda: select_csv_file(app_instance))
    ...
    widgets['reset_button'].grid(row=0, column=1, sticky='ew', padx=(5, 0))
    ...
    widgets['log_area'].grid(row=0, column=0, sticky='nsew', padx=10, pady=10)
...
def reset_ui(app):
    if messagebox.askokcancel("Reset Form?", "Are you sure?"):
        ...
        if "wc_gen" in app.csv_paths: app.csv_paths["wc_gen"] = None
...
def start_automation(app):
    if not hasattr(app, 'csv_paths') or "wc_gen" not in app.csv_paths or not app.csv_paths["wc_gen"]:
...
def run_automation_logic(app):
    ...
    try:
        ...
        app.log_message(widgets['log_area'], "ERROR: CSV file not found.", "error")
    except Exception as e:
