# tabs/mb_entry_tab.py (Upgraded to CustomTkinter)
import tkinter
...
def create_tab(parent_frame, app_instance):
    """Creates the e-MB Entry tab UI with CustomTkinter."""
    ...
    for key, text, r, c in fields:
        ...
        ctk.CTkLabel(config_frame, text=text).grid(row=r, column=c, sticky='w', padx=15, pady=5)
        var = ctk.StringVar()
        widgets['config_vars'][key] = var
        entry = ctk.CTkEntry(config_frame, textvariable=var)
        entry.grid(row=r, column=c+1, columnspan=colspan, sticky='ew', padx=15, pady=5)

    note_label = ctk.CTkLabel(config_frame, text="ℹ️ Note: The script automatically fills the activity for 'Earth work'. If not found, it defaults to the first activity row.", text_color="gray50", wraplength=450)
    note_label.grid(row=5, column=0, columnspan=4, sticky='w', padx=15, pady=(10, 15))
    ...
    widgets['stop_button'] = ctk.CTkButton(action_frame, text="Stop", command=lambda: app_instance.stop_events["mb_entry"].set(), state="disabled", fg_color="gray50")
    ...
    work_codes_frame = notebook.add("Work Codes")
    ...
    widgets['work_codes_text'] = ctk.CTkTextbox(work_codes_frame, wrap=tkinter.WORD)
    ...
    # --- Logs & Status Tab ---
    logs_frame.grid_columnconfigure(1, weight=1); logs_frame.grid_rowconfigure(2, weight=1)
    def on_copy_log_click():
        log_content = widgets['log_display'].get("1.0", tkinter.END); parent_frame.clipboard_clear()
        parent_frame.clipboard_append(log_content); messagebox.showinfo("Copied", "Log content has been copied.")
    widgets['copy_log_button'] = ctk.CTkButton(logs_frame, text="Copy Log", width=100, command=on_copy_log_click)
...
def reset_ui(app):
    if messagebox.askokcancel("Reset Form?", "Are you sure you want to clear all inputs, results, and logs?"):
        for key, var in widgets['config_vars'].items():
            var.set("")
        widgets['config_vars']['measurement_date'].set(datetime.now().strftime('%d/%m/%Y'))
        widgets['work_codes_text'].configure(state="normal"); widgets['work_codes_text'].delete("1.0", tkinter.END)
        widgets['work_codes_text'].configure(state="disabled")
...
def set_ui_state(running):
    ...
    widgets['work_codes_text'].configure(state=state)
    config_frame = widgets['start_button'].master.master # Navigate to config_frame
...
def start_automation(app):
    ...
    if missing_fields:
        ...
        return
    work_codes_raw = [line.strip() for line in widgets['work_codes_text'].get("1.0", tkinter.END).strip().splitlines() if line.strip()]
...
# Logic functions (_process_single_work_code, run_automation_logic, etc.) remain unchanged
# as they interact with Selenium, not the UI, but are included for completeness.
